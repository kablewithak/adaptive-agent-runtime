...
def _inputs() -> tuple[M1DCaseInput, ...]:
    cases = load_m1d_public_cases(_repo_root())
    result: list[M1DCaseInput] = []
    for case in cases:
        result.append(
            M1DCaseInput(
                case=case,
...
def _result_for_case(item: M1DCaseInput, index: int) -> ModelResult:
    ticket = next(
...
def test_m1d_identity_is_frozen_to_deepseek_flash_profile() -> None:
    assert M1D_IDENTITY.profile_name == "deepseek-flash-openai"
    assert M1D_IDENTITY.model_id == "deepseek-v4-flash"
    assert M1D_IDENTITY.stage_label == "M1D"


def test_m1d_runs_same_frozen_suite_with_deepseek_flash(tmp_path: Path) -> None:
    inputs = _inputs()
    ...
    receipt = run_m1d_experiment(
        ...
        profile=_profile(),
        run_id="m1d-test",
        evidence_dir=tmp_path / "m1d-test",
    )

    assert receipt.status is M1DExperimentStatus.COMPLETE
    ...
    assert receipt.frozen_configuration.profile_name == "deepseek-flash-openai"
    assert receipt.frozen_configuration.model_id == "deepseek-v4-flash"
    ...
    assert receipt.schema_version == "m1d-v1"
    assert receipt.frozen_configuration.schema_version == "m1d-deepseek-flash-baseline-v1"

    summary = (tmp_path / "m1d-test" / "summary.json").read_text(encoding="utf-8")
    assert "acceptable_terminal_predicates" not in summary


def test_m1d_rejects_wrong_profile_before_provider_or_evidence(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match="M1D is frozen to profile deepseek-flash-openai",
    ):
        run_m1d_experiment(
            inputs=_inputs(),
            provider=provider,
            profile=_wrong_profile(),
            run_id="m1d-wrong-profile",
