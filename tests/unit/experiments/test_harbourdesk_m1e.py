...
def _profile() -> EndpointProfile:
    return EndpointProfile(
        profile_name="deepseek-pro-openai",
...
def _wrong_profile() -> EndpointProfile:
    return _profile().model_copy(update={"profile_name": "primary-openai", "model_id": "glm-5.1"})
...
def _inputs() -> tuple[M1ECaseInput, ...]:
    cases = load_m1e_public_cases(_repo_root())
    result: list[M1ECaseInput] = []
    for case in cases:
        result.append(
            M1ECaseInput(
                case=case,
...
def _result_for_case(item: M1ECaseInput, index: int) -> ModelResult:
    ticket = next(
...
def test_m1e_identity_is_frozen_to_deepseek_pro_profile() -> None:
    assert M1E_IDENTITY.profile_name == "deepseek-pro-openai"
    assert M1E_IDENTITY.model_id == "deepseek-v4-pro"
    assert M1E_IDENTITY.stage_label == "M1E"


def test_m1e_runs_same_frozen_suite_with_deepseek_pro(tmp_path: Path) -> None:
    inputs = _inputs()
    ...
    receipt = run_m1e_experiment(
        ...
        profile=_profile(),
        run_id="m1e-test",
        evidence_dir=tmp_path / "m1e-test",
    )

    assert receipt.status is M1EExperimentStatus.COMPLETE
    ...
    assert receipt.frozen_configuration.profile_name == "deepseek-pro-openai"
    assert receipt.frozen_configuration.model_id == "deepseek-v4-pro"
    ...
    assert receipt.schema_version == "m1e-v1"
    assert receipt.frozen_configuration.schema_version == "m1e-deepseek-pro-baseline-v1"

    summary = (tmp_path / "m1e-test" / "summary.json").read_text(encoding="utf-8")
    assert "acceptable_terminal_predicates" not in summary


def test_m1e_rejects_wrong_profile_before_provider_or_evidence(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match="M1E is frozen to profile deepseek-pro-openai",
    ):
        run_m1e_experiment(
            inputs=_inputs(),
            provider=provider,
            profile=_wrong_profile(),
            run_id="m1e-wrong-profile",
